...
if ($col < 1) { 
   print STDERR "ERROR the column number should be 1 based counting\n";
   exit 2;
}
my @gene;
...
while (<FH>) {
   ...
   my @f = split(/\t/);
   if (scalar @f < $col) {
      print STDERR "ERROR there is no column $col in $in\n";
      exit 2;
   }
...
if (scalar @gene > 400) {
   print STDERR "ERROR David only allows 400 genes submitted via a link\n";
   exit 2;
}
...
#print output
if (length $link > 2048) { 
   print STDERR "ERROR too many genes to fit in URL, please select a smaller set\n";
   exit 2;
}
